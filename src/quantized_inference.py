"""
Runs manual inference using quantized (fixed-point-simulated or INTn)
weights and activations, and compares accuracy against the float32
baseline.
Run from project root: python3 src/quantized_inference.py
"""
...
# pick a random, fixed subset instead of always the first N images
...
def forward_pass(image, weights, int_bits=None, frac_bits=None, n_bits=None):
    ...
    - If int_bits/frac_bits are given: uses static Q-format fixed-point.
    - If n_bits is given: uses data-driven INTn quantization.
    - If neither: float32 baseline, no quantization.
    """
    def q(x):
    ...
    x = q(np.tanh(conv2d(image, c1_w, c1_b, padding="same")))

    x = q(avgpool2d(x, 2, 2))

    c3_w, c3_b = weights["C3"]
    x = q(np.tanh(conv2d(x, c3_w, c3_b, padding="valid")))
    ...
    x = q(dense(x, c5_w, c5_b, activation="tanh"))

    f6_w, f6_b = weights["F6"]
    x = q(dense(x, f6_w, f6_b, activation="tanh"))

    out_w, out_b = weights["Output"]
    x = dense(x, out_w, out_b, activation="softmax")  # keep softmax unquantized for now
...
def compute_memory_bytes(weights_dict, bits_per_value):
    """Total memory needed to store all weights+biases at a given bit width."""
    total_values = 0
    for name, (w, b) in weights_dict.items():
        total_values += w.size + b.size
    return total_values * bits_per_value / 8  # bits -> bytes


def compute_weight_error(original_weights, quantized_weights):
    """Max/mean/RMSE across ALL weight+bias values, original vs quantized."""
    all_orig, all_quant = [], []
...
# ---- Run full comparison and collect results ----
results = []
...
with open("results/quantization_comparison.csv", "w", newline="") as f:
