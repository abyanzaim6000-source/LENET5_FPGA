...
int main() {
    ...
    // Same hand-computable setup as conv_c3_tb.cpp: all-ones input, all-ones
    // weights, zero bias. With "valid" padding every output pixel -- even
    // the corners -- sees a full K*K*IN_C window, so every output should be
    // exactly K*K*IN_C = 5*5*6 = 150.
    ...
    conv_c3_partialsum(input, weights, bias, output);
