...
int main() {
    ...
    // Pre-activation sum for every output neuron = N_IN = 120 (bias 0 +
    // 120 taps of 1*1). ReLU passes positive values through unchanged,
    // so every output should be exactly 120.0f.
    ...
    dense_f6(input, weights, bias, output);

    // Check the first output neuron -- should be exactly 120.0 (ReLU passthrough)
